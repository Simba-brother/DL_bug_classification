...
def collect_combinewords(text_list:list[str], min_support:float=0.1) -> pd.DataFrame:
    stop_words = set(nltk.corpus.stopwords.words("english"))
    ...
    # support=0.1 表示 itemset 出现次数需要超过 10% transaction 对应的最小计数。
    min_count = max(1, math.ceil(len(transactions) * min_support))
    item_counter = Counter()
    for transaction in transactions:
        item_counter.update(transaction) # 得到每个词出现出现在transactions的次数
    ...
    while current_frequent_itemsets:
        ...
        # 保留同时出现在同一条 Text 中、且出现次数超过阈值的候选 itemsets。
        current_frequent_itemsets = set()
        for candidate, count in candidate_counter.items():
            if count > min_count:
                current_frequent_itemsets.add(candidate)
                combineword_count2itemsets.setdefault(count, []).append(tuple(sorted(candidate)))
        k += 1

    res = {
        count: sorted(itemsets, key=lambda itemset: (len(itemset), itemset))
        for count, itemsets in sorted(combineword_count2itemsets.items(), reverse=True)
    }

    total_count = 0
    for count, itemsets in res.items():
        total_count += len(itemsets)
        print(f"同时出现次数 {count}: {len(itemsets)} 个词组")
    print(f"频繁词组总数量: {total_count}")
    ...
    save_dir = os.path.join(exp_root_dir, "remove_combineword")
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, "combinewords.csv")
    res_df = pd.DataFrame(rows, columns=["ID", "词组", "共同出现次数"])
    res_df.to_csv(save_path, index=False)
    print(f"频繁词组CSV保存在:{save_path}")
    return res_df
...
def get_remove_combineword_dataset():
    ...
    save_dir = os.path.join(exp_root_dir, "remove_combineword","datasets")
    os.makedirs(save_dir, exist_ok=True)

    for _, row in combinword_df.iterrows():
        combineword_id = int(row["ID"])
        words = str(row["词组"]).split()
        pattern = re.compile(
            rf"\b(?:{'|'.join(re.escape(word) for word in words)})\b",
            re.IGNORECASE,
        )
        ...
        save_path = os.path.join(save_dir, f"{combineword_id}_dataset.csv")
        removed_df.to_csv(save_path, index=False)

    print(f"词组删除数据集保存在:{save_dir}")

...
def query_with_retry(client, query_model, query, sample_id):
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        try:
            ...
            return response, time.monotonic() - start_time, None
            ...
            if attempt >= MAX_ATTEMPTS or not is_retryable_error(error):
                return "", time.monotonic() - start_time, error_message
...
def chat(df, llm_name, exp_data_dir):
    ...
    result_dir = os.path.join(exp_data_dir, f"{llm_name}_res")
    ...
    try:
        while pending_rows:
            ...
            for row in pending_rows:
                ...
                total_times[sample_id] = (
                    total_times.get(sample_id, 0.0) + run_time
                )
                ...
                with open(
                    ...
                    time_file.write(str(total_times[sample_id]))

                print(
                    f"{sample_id}\t{status}\t{response}\t"
                    f"{row.Label}\t{total_times[sample_id]}",
                    flush=True,
                )
...
def main():
    ...
    chat(test_df, llm_name, exp_data_dir)
